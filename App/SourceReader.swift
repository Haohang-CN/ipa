...
@MainActor final class SourceReader: NSObject, ObservableObject, WKNavigationDelegate {
    ...
    func read(_ url: String, refresh: Bool = false, maxAttempts: Int = 40) async throws -> [String: Any] {
        ...
        for _ in 0..<maxAttempts {
            ...
            if let data = try? await evaluate(js), !(data["items"] as? [Any] ?? []).isEmpty || !(data["episodes"] as? [Any] ?? []).isEmpty {
                // 网站图片是懒加载的。先让浏览器滚动并读取真实地址，再缓存列表。
                var enriched = data
                if !(data["items"] as? [Any] ?? []).isEmpty {
                    for step in 0..<6 {
                        try Task.checkCancellation()
                        _ = try? await web.evaluateJavaScript("document.querySelectorAll('img').forEach(i => { i.loading='eager'; }); window.scrollTo(0, \(step * 650));")
                        try await Task.sleep(nanoseconds: 500_000_000)
                        if let updated = try? await evaluate(js), !(updated["items"] as? [Any] ?? []).isEmpty { enriched = updated }
                    }
                }
                cache[url] = enriched; return enriched
            }
        }
    ...
    func search(_ query: String) async throws -> [String: Any] {
        ...
        for _ in 0..<40 {
            ...
            if web.isLoading { continue }
            if !submitted {
                let baseline = try? await evaluate(script("extract"))
                previousURL = baseline?["url"] as? String ?? ""
                previousFingerprint = baseline?["fingerprint"] as? String ?? ""
                let result = try? await evaluate(js)
                submitted = result?["state"] as? String == "submitted"
                continue
            }
            guard let result = try? await evaluate(script("extract")),
                  let items = result["items"] as? [[String: Any]], !items.isEmpty else { continue }
            let changed = (result["url"] as? String ?? "") != previousURL ||
                (result["fingerprint"] as? String ?? "") != previousFingerprint
            if !changed { continue }
            let matches = items.filter { ($0["title"] as? String ?? "").localizedStandardContains(query) }
            if !matches.isEmpty {
                var filtered = result
        ...
        throw ReaderError.message("没有找到匹配的影片，或网站未返回搜索结果；请缩短关键词后重试")
    }
